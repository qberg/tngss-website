...
export default function ParallelScroll() {
  ...
  return (
    ...
    >
      <div
        ref={globalpavilion}
        className=' w-full h-full  bg-white flex flex-col justify-center overflow-hidden h-60vh '
      >
        <div className='flags  flex w-full  gap-3 will-change-transform md:flex-nowrap md:overflow-hidden overflow-y-visible overflow-x-auto'>
          {data.map((item, index) => (
            ...
            >
              ...
              >
                <div
                  className='flex flex-col justify-end w-full h-full rounded-2xl'
                  style={{
                    background:
                      'linear-gradient(to top, rgba(0, 0, 0, 0.8) 5%, rgba(0, 0, 0, 0))',
                  }}
                >
