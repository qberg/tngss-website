...
// This component now accepts a 'data' prop which is an array of card objects
export default function AtendeesSection({ data }) {
...
  // The data is now received via props.
...
      ScrollTrigger.getAll().forEach(trigger => trigger.kill());
    };
  }, [isMobile]); // isMobile added to dependencies

  // Added a check to prevent errors if data is not available yet
  if (!data) {
    return null;
  }
