...
// export default function CMSection({cmSectionData}) {
//   return (
//     <div
//       style={{
//         background:
...
// The component now accepts the cmSectionData prop
export default function CMSection({ cmSectionData }) {
  // Defensive check to ensure data is available before rendering
  if (!cmSectionData) {
    return null; // Or a loading spinner/placeholder
  }

  // Destructure the data for cleaner access
  const { heading, image, sub_heading } = cmSectionData;

  // Split the heading to separate the main quote from the name
  // The API response has a double space "  - " separating the quote and the name.
  const [quote, nameWithPrefix] = heading.split("  - ");

  // Create a version of the quote that includes the styled span
  const styledQuote = quote.replace(
    "Number One State",
    `<span class="animate-floatUpBounce bg-gradient-to-r from-blue-500 to-blue-200 text-black px-2 py-0.5 rounded-md">Number One State</span>`
  );
  ...
  return (
    ...
    >
      ...
      >
        {/* Text Section with dynamic content and original design */}
        <div className="flex flex-col items-start justify-center w-full md:w-1/2 space-y-1 px-4 md:px-6 text-left mb-4 md:mb-0">
          <p 
            className="text-base md:text-4xl" 
            dangerouslySetInnerHTML={{ __html: styledQuote }} 
          />
        ...
        <div className="w-full md:w-1/2 h-1/2 md:h-full flex items-center justify-center">
          <img
            alt="Chief Minister"
            src={image || CM} // Use the API image URL or fallback
            className="h-[40%] max-w-3xl max-h-72 md:max-h-full object-contain object-center z-10"
          />
        </div>
      </section>
