...
export default function HeroSection({ className = "", data }) {
  const heroRef = useRef(null)

  useEffect(() => {
    if (heroRef.current) {
      // Any animation or DOM-related logic can go here
    }
  }, [data]);
  ...
  // CRITICAL CHANGE: Split the title into three parts
  const [firstLine, secondLineWithSubtitle] = bannerTitle.split("  ");
  const [firstPart, secondPart] = firstLine.split(" SUMMIT - ");
  const thirdPart = secondLineWithSubtitle;

  // Let's create a more robust splitting logic
  const mainTitleParts = bannerTitle.split('  '); // Splits into "GLOBAL STARTUP SUMMIT - 2025" and "DISRUPT TO RISE"
  const firstLineText = mainTitleParts[0];
  const secondLineText = mainTitleParts[1];

  const firstPartText = firstLineText.split(' SUMMIT')[0]; // "GLOBAL STARTUP"
  const secondPartText = 'SUMMIT' + firstLineText.split(' SUMMIT')[1]; // "SUMMIT - 2025"
  
  const bannerImages = data.banner_image || [];
  const mainLogo = bannerImages[1];
  const smallLogo = bannerImages[0];
  ...
  return (
    <>
      ...
      >
        <video
          className="absolute w-full h-full inset-0 z-10 object-cover opacity-60"
          src={data.banner_video || "https://divsh6mubpk9o.cloudfront.net/hero-vid.mp4"}
          autoPlay
          muted
          playsInline
          loop
        ...
        <div className="relative herotxt z-20 md:mt-20 will-change-transform">
          ...
          >
            ...
            <div className="logo-3d-wrapper hero-animate-floatUpBounce mt-8">
              <img
                alt="StartupTN Logo"
                src={data.banner_image [1]|| herobannerlogo}
                style={{ maxWidth: "50%" }}
                className="object-contain heroBg logo-3d-img mt-5"
              />
            </div>
            ...
            >
              ...
              <div className="w-full">
                <img
                  src={data.banner_image [0]|| startupwhite}
                  alt="Decorative Shape"
                  className=""
                  style={{ maxWidth: "60%" }}
                />
              ...
              {/* Text Block - MODIFIED for three sizes */}
              <div className="flex flex-col items-start md:items-start text-left w-full ">
                <p className="text-white font-bold text-left leading-tight">
                  <span className="font-montserrat font-bold block text-2xl md:text-7xl">
                    {firstPartText}
                  </span>
                  <span className="font-montserrat font-bold block text-xl md:text-5xl">
                    {secondPartText}
                  </span>
                </p>
                <p
        ...
        {/* Location */}
        <p className="hero-animate-fadeIn-delay-1 text-center text-3xl md:text-5xl mb-5 md:mb-7 mt-8 md:mt-12 z-30">
          <img className="w-6 md:w-9 inline mb-1 mr-1 text-white" src={pin || "/placeholder.svg"} />
        ...
        <div className="hero-animate-fadeIn-delay-3 text-white text-2xl flex justify-center align-end py-3 z-20 flex flex-col mb-12 ">
          <CTAButton src="https://event.startuptn.in/" className="rounded-2xl hover:scale-105">
            <div className="w-70 h-10 px-6 py-7 flex items-center justify-center">
              <img className="px-2" src={vector || "/placeholder.svg"} /> {data.banner_button_text}
            </div>
          </CTAButton>
          <CTAButton src="https://event.startuptn.in/" className="rounded-2xl hover:scale-105 md:hidden">
            <div className="w-70 h-10 px-6 py-7 flex items-center justify-center">
              <img
                className="px-2 inline-block w-10 h-5"
                src={book || "/placeholder.svg"}
