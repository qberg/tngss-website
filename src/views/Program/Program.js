...
//       setLoading(true)
//     }
//     // "http://192.168.1.15:8003/event-service/v1/speakers/find-all-web"
//     // "https://dev.tngss.startuptn.in/event-service/v1/events/find-all"
//     try {
//       const response = await axios.get("http://192.168.1.15:8003/event-service/v1/events/find-all-web", {
//         headers: {
//           Accept: "application/json",
//           "Content-Type": "application/json",
//         },
...
function App() {
  ...
  const fetchEvents = async (showLoading = true) => {
    ...
    try {
      const response = await axios.get("https://dev.tngss.startuptn.in/event-service/v1/events/find-all-web", {
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        },
  ...
  const handleDateSelect = async (fullDate) => {
    ...
    try {
      const formattedDate = moment(fullDate, "DD MMMM YYYY").format("YYYY-MM-DD");
      const response = await axios.get(
      ...
      setProcessedEvents(prev => ({ ...prev, [fullDate]: { main: [], partner: [] } }));
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [apiEvents, selectedTag, showPartnerEvents, selectedSubcategories, selectedMonth]); // `apiEvents` is here to trigger when the full data set changes due to other filters or initial load.
