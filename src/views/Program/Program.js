...
const FilterDropdown = ({ options, selected, onChange }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute z-10 mt-1 w-full bg-gray-800 border border-gray-700 rounded-md shadow-lg max-h-60 overflow-y-auto"> {/* Added max-h-60 and overflow-y-auto */}
                    <ul className="py-1">
                        {options.map((option) => (
                            <li key={option}>
                                <button
