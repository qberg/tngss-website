            ...
            <div className='space-y-0 mt-4'>
              {subcategories.map((subcategory) => (
                <div
                  ...
                  onClick={() =>
                    onSubcategoryChange(subcategory._id, subcategory.name)
                  }
                >
  ...
  return (
    <div className='bg-black text-white'>
      ...
      <div className='px-4'>
        {/* Tags Section */}
        <div className='mb-6'>
          <h3 className='text-lg font-medium mb-3' style={{ color: '#17BFDB' }}>
            Tags
          </h3>
          <FilterDropdown
            options={tagOptions}
        ...
        <div className='bg-[#1A1A1A]/90 border border-gray-800 rounded-lg overflow-hidden shadow-md'>
          <div className='p-4'>
            ...
            <p className='text-white text-bold'>
              ...
              {event.location && event.location.join(', ')}
            </p>
          </div>
          <div className='border-t border-gray-800 p-4'>
            <h4 className='text-[#17BFDB] text-xl font-semibold mb-2 get-text'>
              SPEAKERS
            </h4>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <div className='text-md text-white'>
              {event.host_details && event.host_details[0] && (
                <>
                  ...
                  <p className='text-white text-bold'>
                    {event.host_details[0].designation}
                  </p>
                </>
              )}
            </div>
...
function App() {
  ...
  // `apiEvents` will now hold the full list of events, not filtered by date clicks
  const [apiEvents, setApiEvents] = useState([])
  // `processedEvents` will be the structured events for display
  ...
  useEffect(() => {
    ...
    if (allSelectedSubcategoryNames?.length === 0) {
      ...
      if (!loading) {
        // If no subcategories are selected, refetch all events (for a potential full list refresh)
    ...
    axios
      .get(
        `https://dev.tngss.startuptn.in/event-service/v1/events/find-all?sub_category=${subcategoryParams}`,
        {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
      ...
      .then((response) => {
        // When subcategories are selected, these become the primary source for `apiEvents`
  ...
  }, [selectedSubcategories, loading]) // Keep loading here for initial load condition

  // Fetch ALL events initially or when filters are reset
  const fetchEvents = async (showLoading = true) => {
    ...
    try {
      const response = await axios.get(
        'https://dev.tngss.startuptn.in/event-service/v1/events/find-all-web',
        {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
      ...
      setApiEvents(response.data.data.events)
    } catch (err) {
      setError(err)
  ...
  const handleDateSelect = async (fullDate) => {
    setActiveFullDate(fullDate)
    setLoading(true)

    try {
      const formattedDate = moment(fullDate, 'DD MMMM YYYY').format(
        'YYYY-MM-DD'
      )
      const response = await axios.get(
        `https://dev.tngss.startuptn.in/event-service/v1/events/find-by-date?start_date=${formattedDate}`,
        {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
      ...
      const fetchedEventsForDate = response.data.data.events || []

      // Manually categorize and set processedEvents for ONLY the specific clicked date
      // This will ensure only events from the date API are shown for this date
      const newProcessedEvents = { ...processedEvents } // Start with existing processed events
      newProcessedEvents[fullDate] = { main: [], partner: [] } // Initialize for the clicked date

      fetchedEventsForDate.forEach((event) => {
        if (event.event_for === 'main') {
          newProcessedEvents[fullDate].main.push(event)
        } else if (event.event_for === 'partner') {
          newProcessedEvents[fullDate].partner.push(event)
        }
      })
      setProcessedEvents(newProcessedEvents)

      // We don't call setApiEvents here, to avoid re-triggering the main processing useEffect
      // with a filtered list of events.
    } catch (err) {
      setError(err)
      console.error('Error fetching events by date:', err)
      // If there's an error, clear events for that specific date in processedEvents
      setProcessedEvents((prev) => ({
        ...prev,
        [fullDate]: { main: [], partner: [] },
      }))
    } finally {
      setLoading(false)
    }
  ...
  // This useEffect processes the `apiEvents` whenever filters (tags, subcategories, month, event type) change.
  // IMPORTANT: `apiEvents` should be the *full* list of events (from `fetchEvents` or subcategory filter),
  // not the result of a single date API call.
  useEffect(() => {
    const categorizedEvents = {}
    const uniqueDates = new Set()

    // Filter `apiEvents` by selected month and tag
    const monthFilteredEvents = apiEvents?.filter((event) => {
      const startDateMoment = moment(event.start_date)
      // For main events, show July (month 6, 0-indexed), for partner events show selected month
      const targetMonth = showPartnerEvents ? selectedMonth - 1 : 6
      return (
        startDateMoment.month() === targetMonth &&
        startDateMoment.year() === 2025
      )
    })

    const tagAndTypeFilteredEvents = monthFilteredEvents?.filter((event) => {
      const tagMatch =
        selectedTag === 'ALL' ||
    ...
    tagAndTypeFilteredEvents?.forEach((event) => {
      const startDateMoment = moment(event.start_date)
      ...
      uniqueDates.add(
        JSON.stringify({ day: dayKey, date: dateNum, fullDate: fullDate })
      )
    ...
    const sortedDates = Array.from(uniqueDates)
      ...
      .sort(
        (a, b) =>
          moment(a.fullDate, 'DD MMMM YYYY').valueOf() -
          moment(b.fullDate, 'DD MMMM YYYY').valueOf()
      )

    setAvailableDates(sortedDates)

    // Only update processedEvents if this useEffect is not triggered by a direct date selection
    // The `handleDateSelect` function now handles setting processedEvents directly for a chosen date.
    // This prevents the data from "snapping back"
    // Also, ensure `processedEvents` is updated if filters (like tags) change after a date has been selected.
    if (activeFullDate && categorizedEvents[activeFullDate]) {
      setProcessedEvents(categorizedEvents)
    } else if (!activeFullDate && sortedDates?.length > 0) {
      // Set the active date to the first available date if no date is active
      ...
      setProcessedEvents(categorizedEvents) // Default behavior if no specific date is active
    }
  }, [
    apiEvents,
    selectedTag,
    ...
    selectedSubcategories,
    selectedMonth,
  ]) // `apiEvents` is here to trigger when the full data set changes due to other filters or initial load.
  ...
  const handleMonthChange = (month) => {
    ...
    setActiveFullDate('') // Reset selected date when month changes, so the date selector defaults to the first available date of the new month
  }
    ...
    <div className='min-h-screen bg-black text-white font-sans'>
      ...
      <div className='max-w-7xl mx-auto px-4 py-8 p-5'>
        <div className='space-y-4 mb-8'>
          ...
          <div className='flex flex-col sm:flex-row sm:items-center sm:justify-end space-y-4 sm:space-y-0 bg-gray-600 pl-3 pr-3 p-2'>
            {availableDates?.length > 0 && (
              <div className='sm:mr-auto'>
                <DateSelector
                  activeFullDate={activeFullDate}
                  setActiveFullDate={handleDateSelect}
            ...
            <div className='rounded-full bg-white inline-flex'>
              <button
                className={`px-6 py-3 text-sm rounded-full transition
                  ${
                    !showPartnerEvents
                      ? 'btn-active shadow-2xl'
                      : 'btn-inactive'
                  }`}
                onClick={() => {
                  setShowPartnerEvents(false)
                  setSelectedMonth(7)
                  // Optionally reset activeFullDate here if you want to default to the first Main event date
                  setActiveFullDate('') // This will make the useEffect pick the first main event date
                }}
              >
                Main events
              </button>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-4 gap-6'>
          ...
          <div className='lg:col-span-3'>
            <div className='rounded-md p-3 mb-8 bg-gradient-blur h-12'>
              <div className='flex items-center text-lg font-medium justify-center'>
                {activeFullDate && <Calendar className='mr-2 h-5 w-5' />}
              ...
              <div className='flex flex-col items-center justify-center py-20'>
                ...
                <p className='text-xl text-gray-500'>
                  {hasSelectedSubcategories
                    ...
                    : `No ${
                        showPartnerEvents ? 'partner' : 'main'
                      } events available for ${
                        activeFullDate || 'the selected date'
                      }.`}
                </p>
              </div>
            )}
