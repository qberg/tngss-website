...
// // http://LAPTOP-7G6U4695:8002/tngss/faq/find-all
//       try {
//         const response = await fetch(` http://192.168.1.21:8003/event-service/v1/faq/find-all`);
        ...
        const fetchData = async () => {
            try {
              // https://dev.tngss.startuptn.in
                const response = await fetch(`https://dev.tngss.startuptn.in/cms-service/v1/faq/find-all`);
                const data = await response.json();
                // Ensure data.data and data.data.FAQs exist before setting state
                if (data && data.data && data.data.FAQs) {
                    setFaqSections(data.data.FAQs);
                } else {
                    console.warn('API response does not contain expected FAQ data structure:', data);
                    setFaqSections([]); // Set to empty array to avoid errors
                }
            } catch (error) {
                console.error('Error fetching data:', error);
                setFaqSections([]); // Set to empty array on error
            }
        };
            ...
            <div className="flex flex-col items-center py-10 px-5 bg-black w-full">
                {faqSections.map((section, index) => (
                    <FaqSection
                        ...
                        title={section.category}
                        items={section.questions.map(question => ({
                            id: question._id, // **CHANGED: Use question._id for a unique key**
                            title: question.title, // **CHANGED: Use lowercase 't' for title**
                            content: question.answer // **CHANGED: Use lowercase 'a' for answer**
                        }))}
                    />
                ))}
            </div>
                            ...
                            <div className="flex justify-start">
                                <button
                                    ...
                                    style={{
                                        ...
                                        backgroundImage: 'linear-gradient(#1a1a1a, #1a1a1a), linear-gradient(to right, #0055FF, #18BFDB, #F5710C, #EC473E)',
                                        backgroundOrigin: 'border-box',
                                        backgroundClip: 'padding-box, border-box',
                                        letterSpacing:"0.9px"
                                    }}
        ...
        <section className="my-5 bg-black rounded-3xl md:rounded-[50px] w-full max-w-full overflow-hidden gradient-border mx-4 sm:mx-6">
            <div className='gradient-inner p-6 md:p-10'>
                <header className="flex items-center mb-5">
                    ...
                    <h2 className="text-3xl md:text-6xl sm:text-3xl font-bold text-[#0099ff] text-gradient-1" style={{lineHeight:"6rem"}}>
                        {title}
                    </h2>
                </header>
                <h3 className="mb-5 text-2xl md:text-4xl font-light text-white ml-4">FAQs</h3>
